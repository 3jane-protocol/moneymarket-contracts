...
/**
 * @title ExecuteUSD3AtomicBatch
 * @notice Execute USD3 upgrade as atomic batch via Safe multisig
 ...
 */
contract ExecuteUSD3AtomicBatch is Script, SafeHelper, TimelockHelper {
    ...
    function run(bytes32 operationId, uint256 previousProfitUnlockTime, bool send)
        ...
        isTimelock(TIMELOCK)
    {
        console2.log("=== Executing USD3 Atomic Batch via Safe ===");
        ...
        // 4. Timelock.executeBatch() -> ProxyAdmin.upgradeAndCall() + USD3.reinitialize()
        console2.log("4. Timelock.executeBatch() -> ProxyAdmin.upgradeAndCall() + USD3.reinitialize()");
