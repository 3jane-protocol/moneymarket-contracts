...
/**
 * @title ScheduleIRMUpgrade
 * @notice Schedule AdaptiveCurveIrm upgrade through TimelockController via Safe multisig
 * @dev Part of v1.1 upgrade process
 *
 *      Usage:
 *      1. Deploy new AdaptiveCurveIrm implementation (script 11)
 *      2. Run this script to schedule upgrade
 *      3. Wait 2 days for timelock delay
 *      4. Execute upgrade via ExecuteTimelockViaSafe.s.sol
 */
contract ScheduleIRMUpgrade is Script, SafeHelper, TimelockHelper {
    /// @notice TimelockController address (mainnet)
    address private constant TIMELOCK = 0x1dCcD4628d48a50C1A7adEA3848bcC869f08f8C2;

    /// @notice AdaptiveCurveIrm proxy address (mainnet)
    address private constant IRM_PROXY = 0x1d434D2899f81F3C3fdf52C814A6E23318f9C7Df;

    /// @notice AdaptiveCurveIrm ProxyAdmin address (mainnet)
    address private constant PROXY_ADMIN = 0x5B7961DaFce9e412d26d6B92d06A9e0db3E3c7CF;

    /// @notice EIP-1967 admin slot
    bytes32 private constant ADMIN_SLOT = 0xb53127684a568b3173ae13b9f8a6016e243e63b6e8ee1178d6a717850b5d6103;

    /// @notice Main execution function
    /// @param newImplementation Address of the new AdaptiveCurveIrm implementation
        ...
        isTimelock(TIMELOCK)
    {
        ...
        values[0] = 0;
        datas[0] =
            abi.encodeCall(ProxyAdmin.upgradeAndCall, (ITransparentUpgradeableProxy(IRM_PROXY), newImplementation, ""));

        // Generate salt and predecessor
        bytes32 salt = generateSalt("AdaptiveCurveIrm v1.1 Upgrade");
    ...
    function dryRun() external {
        address newImpl = vm.envAddress("IRM_NEW_IMPL");
