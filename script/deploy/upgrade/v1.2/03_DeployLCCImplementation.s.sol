 ...
 * @dev LCCAuctionLib is externally linked. Forge deploys and links it during broadcast; every future implementation
 *      redeploy must deploy and link the library again. The implementation CREATE2 address is deployer-dependent
 *      because the linked library address is embedded in its initcode.
 *
...
contract DeployLCCImplementation is Script, LCCWiringCheck {
    ...
    function run() external returns (address vaultImpl) {
        ...
        console2.log("  1. Verify LCCAuctionLib and LCCVault on Etherscan");
        console2.log("  2. Record the linked LCCAuctionLib address for future implementation upgrades");
