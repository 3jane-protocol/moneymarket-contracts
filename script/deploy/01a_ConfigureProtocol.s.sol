...
contract ConfigureProtocol is Script {
    function run() external {
        ...
        config.setConfig(keccak256("MAX_ON_CREDIT"), 1 ether); // 95% max on credit
        config.setConfig(keccak256("IRP"), 0); // 0% IRP
        config.setConfig(keccak256("MIN_BORROW"), 5000e6); // 5K min borrow
        config.setConfig(keccak256("GRACE_PERIOD"), 1 days); // 7 days grace period
        config.setConfig(keccak256("DELINQUENCY_PERIOD"), 23 days); // 23 days delinquency period
        config.setConfig(keccak256("CYCLE_DURATION"), 30 days); // 30 days cycle duration
