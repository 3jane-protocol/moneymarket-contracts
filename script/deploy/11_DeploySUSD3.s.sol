...
// Don't import flattened sUSD3 directly to avoid duplicate declarations
// The contract will be deployed using the compiled bytecode

// Define the initialization interface
interface ISUSD3Initialize {
    function initialize(address _usd3Token, address _management, address _keeper) external;
}
...
contract DeploySUSD3 is Script {
    function run() external returns (address, address) {
        ...
        if (existing != address(0)) {
            ...
            address impl = _loadAddress("susd3Impl");
            if (impl != address(0)) {
