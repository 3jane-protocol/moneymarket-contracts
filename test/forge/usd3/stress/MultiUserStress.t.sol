...
contract MultiUserStressTest is Setup {
    ...
    event SystemState(uint256 usd3Total, uint256 susd3Total, uint256 subordinationRatio);
    ...
    function test_simultaneousDeposits() public {
        ...
        assertApproxEqAbs(totalAssets, expectedTotal, 1000e6, "Total assets should match sum of deposits");
    }
    ...
    function test_simultaneousWithdrawals() public {
        ...
        for (uint256 i = 0; i < NUM_USERS; i++) {
            ...
            uint256 maxRedeemable = ITokenizedStrategy(address(usd3Strategy)).maxRedeem(user);
            uint256 actualWithdrawShares = desiredWithdrawShares > maxRedeemable ? maxRedeemable : desiredWithdrawShares;

            if (actualWithdrawShares > 0) {
                vm.startPrank(user);
                uint256 assetsReceived =
                    ITokenizedStrategy(address(usd3Strategy)).redeem(actualWithdrawShares, user, user);
                vm.stopPrank();

                assertGt(assetsReceived, 0, "User should receive assets on withdrawal");
        ...
        assertApproxEqRel(totalAssetsAfter, totalAssetsBefore / 2, 0.05e18, "Should withdraw ~50% of assets");
    }
    ...
    function test_mixedOperationsConcurrency() public {
        ...
        for (uint256 i = 2; i < NUM_USERS / 2; i++) {
            ...
            if (usd3Balance > 0) {
                ...
                uint256 depositAmount = usd3Balance > availableLimit ? availableLimit : usd3Balance;
        ...
        uint256 usd3TotalSupply = ITokenizedStrategy(address(usd3Strategy)).totalSupply();
        ...
        emit SystemState(usd3TotalSupply, susd3Balance, (susd3Balance * 100) / usd3TotalSupply);
    }
    ...
    function test_subordinationRatioBoundaryStress() public {
        ...
        uint256 maxSusd3Assets = (usd3TotalAssets * MAX_SUBORDINATION) / (100 - MAX_SUBORDINATION);

        // Multiple users try to deposit up to the limit
        uint256 remainingSusd3Capacity = maxSusd3Assets;
        uint256 susd3UsersCount = 0;

        for (uint256 i = 1; i < NUM_USERS && remainingSusd3Capacity > BASE_AMOUNT; i++) {
            address user = users[i];
            ...
            if (attemptAmount <= remainingSusd3Capacity) {
                ...
                ERC20(address(usd3Strategy)).approve(address(susd3Strategy), usd3SharesToStake);
                ...
                try susd3Strategy.deposit(usd3SharesToStake, user) {
                    ...
                    susd3UsersCount++;
                    emit UserAction(user, "susd3_deposit_success", attemptAmount);
                } catch {
                    emit UserAction(user, "susd3_deposit_failed", attemptAmount);
                }
            }
            vm.stopPrank();
        }
        ...
        uint256 finalUsd3Supply = ITokenizedStrategy(address(usd3Strategy)).totalAssets();
        uint256 finalSusd3Balance = ITokenizedStrategy(address(usd3Strategy)).balanceOf(address(susd3Strategy));
        uint256 finalSusd3Value = ITokenizedStrategy(address(usd3Strategy)).convertToAssets(finalSusd3Balance);
    ...
    function test_subordinationRatioWithVaryingDeposits() public {
        ...
        // Verify final state
        uint256 totalUsd3 = ITokenizedStrategy(address(usd3Strategy)).totalAssets();
        uint256 susd3UsdBalance = ITokenizedStrategy(address(usd3Strategy)).balanceOf(address(susd3Strategy));

        assertGt(totalUsd3, 0, "Should have USD3 assets");

        if (susd3UsdBalance > 0) {
            uint256 susd3Value = ITokenizedStrategy(address(usd3Strategy)).convertToAssets(susd3UsdBalance);
            uint256 ratio = (susd3Value * 100) / totalUsd3;
            assertLe(ratio, MAX_SUBORDINATION, "Final ratio should respect limit");
        }
    }
    ...
    function test_liquidityContentionDuringWithdrawals() public {
        ...
        for (uint256 i = 0; i < NUM_USERS; i++) {
            ...
            vm.startPrank(user);
            try ITokenizedStrategy(address(usd3Strategy)).redeem(withdrawShares, user, user) returns (uint256 assets) {
        ...
        uint256 remainingAssets = ITokenizedStrategy(address(usd3Strategy)).totalAssets();
        assertGe(remainingAssets, 0, "Should have non-negative remaining assets");
    }
    ...
    function test_yieldDistributionWithManyUsers() public {
        ...
        for (uint256 i = NUM_USERS / 2; i < NUM_USERS; i++) {
            ...
            uint256 availableLimit = susd3Strategy.availableDepositLimit(user);
            if (availableLimit > 0) {
                depositAmount = depositAmount > availableLimit ? availableLimit : depositAmount;
                ERC20(address(usd3Strategy)).approve(address(susd3Strategy), depositAmount);
                susd3Strategy.deposit(depositAmount, user);
            }
            vm.stopPrank();
        ...
        uint256 yieldAmount = (ITokenizedStrategy(address(usd3Strategy)).totalAssets() * 5) / 100; // 5% yield
        ...
        for (uint256 i = 0; i < NUM_USERS; i++) {
            ...
            if (usd3Balance > 0) {
                ...
                uint256 maxRedeemable = ITokenizedStrategy(address(usd3Strategy)).maxRedeem(user);
                uint256 actualRedeem = desiredRedeem > maxRedeemable ? maxRedeemable : desiredRedeem;

                if (actualRedeem > 0) {
                    vm.startPrank(user);
                    uint256 withdrawn = ITokenizedStrategy(address(usd3Strategy)).redeem(actualRedeem, user, user);
                    vm.stopPrank();

                    assertGt(withdrawn, 0, "User should be able to withdraw after yield distribution");
                }
            }
        }
    }
    ...
    function test_complexInteractionPattern() public {
        ...
        // Phase 3: Add yield
        uint256 yield1 = (ITokenizedStrategy(address(usd3Strategy)).totalAssets() * 3) / 100;
        ...
        uint256 loss = (ITokenizedStrategy(address(usd3Strategy)).totalAssets() * 2) / 100;
        _simulateLoss(loss);
        ...
        for (uint256 i = 0; i < NUM_USERS / 4; i++) {
            address user = users[i];
            uint256 balance = ITokenizedStrategy(address(usd3Strategy)).balanceOf(user);

            if (balance > 0) {
                vm.startPrank(user);
                ITokenizedStrategy(address(usd3Strategy)).redeem(balance / 3, user, user);
                vm.stopPrank();
            }
        }

        // Verify system integrity after complex interactions
        uint256 finalTotalAssets = ITokenizedStrategy(address(usd3Strategy)).totalAssets();
        ...
        for (uint256 i = 0; i < NUM_USERS; i++) {
            ...
            if (balance > 0) {
                vm.startPrank(user);
                try ITokenizedStrategy(address(usd3Strategy)).redeem(1, user, user) returns (uint256) {
        ...
        assertGt(successfulFinalWithdrawals, 0, "Some users should be able to make final withdrawals");
    }
    ...
    function test_simultaneousLossAndWithdrawals() public {
        ...
        for (uint256 i = NUM_USERS / 2; i < NUM_USERS; i++) {
            ...
            if (balance > 0) {
                vm.startPrank(user);
                try ITokenizedStrategy(address(usd3Strategy)).redeem(balance / 2, user, user) returns (uint256 assets) {
                    successfulWithdrawals++;
                    assertGt(assets, 0, "Should receive some assets even after loss");
                } catch {
        ...
        assertTrue(successfulWithdrawals >= 0, "System should remain functional after loss");
