...
contract EdgeCases is Setup {
    ...
    function testFuzz_depositAmounts(uint256 amount) public {
        ...
        if (amount > 0) {
            ...
            assertEq(IERC20(address(usd3Strategy)).balanceOf(alice), shares, "Alice should receive shares");
        }
        vm.stopPrank();
    }
     ...
     */
    function testFuzz_withdrawalAmounts(uint256 amount, uint256 maxLoss) public {
        ...
        // Try to withdraw
        vm.prank(alice);
        uint256 sharesUsed = usd3Strategy.withdraw(amount, alice, alice, maxLoss);

        // Verify withdrawal succeeded
        assertGt(sharesUsed, 0, "Should use shares for withdrawal");
        assertLe(IERC20(address(usd3Strategy)).balanceOf(alice), shares, "Alice's shares should decrease");
    }
    ...
    function testFuzz_subordinationRatios(uint256 ratio) public {
        ...
        if (ratio == 0) {
            ...
            assertApproxEqAbs(depositLimit, defaultLimit, 100e6, "Should use default ratio when 0");
        } else {
            assertApproxEqAbs(depositLimit, expectedMaxDeposit, 100e6, "Deposit limit should match ratio");
        }
    }
    ...
    function testFuzz_performanceFeeBoundaries(uint256 fee) public {
        ...
        uint256 depositAmount = availableLimit > 3000e6 ? 3000e6 : availableLimit;
        ...
        if (depositAmount > 0) {
            if (fee == 0) {
                assertEq(susd3BalanceAfter, susd3BalanceBefore, "No shares minted with 0% fee");
            } else if (fee == 10000) {
                ...
                if (sharesReceived > 0) {
                    ...
                    assertApproxEqAbs(valueReceived, 1000e6, 10e6, "Should receive ~100% of profit");
                }
            } else {
                // Partial distribution
                uint256 sharesReceived = susd3BalanceAfter - susd3BalanceBefore;
                if (sharesReceived > 0) {
                    assertLt(sharesReceived, 1000e6, "Should receive less than full profit in shares");
                }
            }
        }
    }
    ...
    function testFuzz_lossAbsorption(uint256 lossPercent) public {
        ...
        // Check available limit and deposit within subordination ratio
        uint256 availableLimit = susd3Strategy.availableDepositLimit(bob);
        uint256 depositAmount = availableLimit > 15000e6 ? 15000e6 : availableLimit;
     ...
     */
    function testFuzz_timeParameters(uint256 lockDuration, uint256 cooldownDuration, uint256 withdrawalWindow) public {
        ...
        // Verify lock is set
        assertEq(
            susd3Strategy.lockedUntil(alice), block.timestamp + lockDuration, "Lock should match configured duration"
        );
        ...
        (uint256 cooldownEnd, uint256 windowEnd,) = susd3Strategy.getCooldownStatus(alice);
        assertEq(cooldownEnd, block.timestamp + cooldownDuration, "Cooldown should match configured duration");
        assertEq(
            windowEnd, block.timestamp + cooldownDuration + withdrawalWindow, "Window should match configured duration"
        );
        vm.stopPrank();
    }
    ...
    function test_rapidDepositWithdrawCycles() public {
        ...
        for (uint256 i = 0; i < cycles; i++) {
            ...
            uint256 maxWithdrawable = ITokenizedStrategy(address(usd3Strategy)).maxWithdraw(alice);
            uint256 toWithdraw = cycleAmount > maxWithdrawable ? maxWithdrawable : cycleAmount;
    ...
    function test_concurrentMultiUserOperations() public {
        ...
        assertEq(remainingSupply, 70000e6, "Supply should decrease by withdrawals");
    }
     ...
     */
    function testFuzz_protocolInvariants(uint256 deposits, uint256 withdrawals, uint256 losses) public {
        // Bound inputs
        ...
        assertGe(supply, aliceBalance, "Supply should be at least user balance");

        // Withdraw (respecting limits)
        if (withdrawals > 0 && deposits > 0) {
            uint256 maxWithdrawable = ITokenizedStrategy(address(usd3Strategy)).maxWithdraw(alice);
            uint256 actualWithdrawal = withdrawals > maxWithdrawable ? maxWithdrawable : withdrawals;
        ...
        uint256 totalAssets = ITokenizedStrategy(address(usd3Strategy)).totalAssets();
        uint256 totalSupplyInAssets =
            ITokenizedStrategy(address(usd3Strategy)).convertToAssets(IERC20(address(usd3Strategy)).totalSupply());
        assertGe(
