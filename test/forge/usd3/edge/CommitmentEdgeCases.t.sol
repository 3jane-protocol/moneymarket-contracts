...
contract CommitmentEdgeCasesTest is Setup {
    ...
    function test_partialWithdrawalCommitmentBehavior() public {
        ...
        assertGt(withdrawn, 0, "Should withdraw remaining after new commitment");
        vm.stopPrank();
    }
    ...
    function test_commitmentDuringEmergencyShutdown() public {
        ...
        if (maxRedeemable > 0) {
            ...
            assertGt(withdrawn, 0, "Should emergency withdraw during commitment");
        } else {
    ...
    function test_zeroCommitmentPeriod() public {
        ...
        address protocolConfigAddress = MorphoCredit(morphoAddress).protocolConfig();
        bytes32 USD3_COMMITMENT_TIME = keccak256("USD3_COMMITMENT_TIME");
        MockProtocolConfig(protocolConfigAddress).setConfig(USD3_COMMITMENT_TIME, 0);
    ...
    function test_commitmentPeriodChangeEffects() public {
        ...
        address protocolConfigAddress = MorphoCredit(morphoAddress).protocolConfig();
        bytes32 USD3_COMMITMENT_TIME = keccak256("USD3_COMMITMENT_TIME");
        MockProtocolConfig(protocolConfigAddress).setConfig(USD3_COMMITMENT_TIME, 3 days);
    ...
    function test_rapidDepositsWithdrawalsAtBoundaries() public {
        ...
        address protocolConfigAddress = MorphoCredit(morphoAddress).protocolConfig();
        bytes32 USD3_COMMITMENT_TIME = keccak256("USD3_COMMITMENT_TIME");
        MockProtocolConfig(protocolConfigAddress).setConfig(USD3_COMMITMENT_TIME, 1 days);
    ...
    function test_minimum_deposit_after_transfer() public {
        ...
        assertGt(bobShares, 0, "Bob can deposit small amount with existing shares");
    ...
    function test_multiple_simultaneous_first_deposits() public {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            assertGt(usd3Strategy.depositTimestamp(users[i]), 0, "Has commitment timestamp");
        }
    }
}
