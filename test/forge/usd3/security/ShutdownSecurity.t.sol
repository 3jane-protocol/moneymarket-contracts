...
contract ShutdownSecurityTest is Setup {
    ...
    function test_shutdown_state_manipulation_attack() public {
        ...
        // Approve for redeem
        IERC20(address(usd3Strategy)).approve(address(usd3Strategy), remainingShares);
    ...
    function test_minimum_deposit_mint_subsequent() public {
        ...
        uint256 sharesToMint = ITokenizedStrategy(address(usd3Strategy)).previewDeposit(10e6);
        ...
        uint256 bobShares = ITokenizedStrategy(address(usd3Strategy)).previewDeposit(100e6);
        ...
        uint256 bobSmallShares = ITokenizedStrategy(address(usd3Strategy)).previewDeposit(5e6);
    ...
    function test_shutdown_insufficient_liquidity() public {
        ...
        if (availableLiquidity < 1000e6) {
            ...
            if (availableLiquidity > 0) {
                uint256 withdrawn = usd3Strategy.withdraw(availableLiquidity, alice, alice);
                assertEq(withdrawn, availableLiquidity, "Should withdraw available amount");
            }
        } else {
    ...
    function test_shutdown_respects_max_loss() public {
        ...
        uint256 currentValue = ITokenizedStrategy(address(usd3Strategy)).previewRedeem(aliceShares);
        ...
        if (currentValue < 1000e6) {
            ...
            uint256 withdrawn = usd3Strategy.redeem(aliceShares, alice, alice, 10_000); // 100% maxLoss
            assertGt(withdrawn, 0, "Should withdraw with appropriate maxLoss");
        } else {
            // No loss, should work with 0 maxLoss
            uint256 withdrawn = usd3Strategy.redeem(aliceShares, alice, alice, 0);
    ...
    function test_double_deposit_attack() public {
        ...
        uint256 maxRedeemable = ITokenizedStrategy(address(usd3Strategy)).maxRedeem(attacker);
        ...
        uint256 remainingShares = IERC20(address(usd3Strategy)).balanceOf(attacker);
        vm.stopPrank();
        ...
        if (remainingShares == 0) {
            ...
            assertGt(newShares, 0, "Can deposit small amounts with existing balance");
            vm.stopPrank();
        }
    }
    ...
    function test_fee_extraction_during_shutdown() public {
        ...
        uint256 feeShares = ITokenizedStrategy(address(usd3Strategy)).balanceOf(management);
        ...
        if (feeShares > 0) {
            ...
            vm.startPrank(management);
            IERC20(address(usd3Strategy)).approve(address(usd3Strategy), feeShares);
            uint256 feeAmount = usd3Strategy.redeem(feeShares, management, management);
    ...
    function test_multi_user_shutdown_race() public {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            if (userShares > 0) {
                // Approve for redeem
                IERC20(address(usd3Strategy)).approve(address(usd3Strategy), userShares);
                withdrawals[i] = usd3Strategy.redeem(userShares, user, user);
                assertGt(withdrawals[i], 0, "User should withdraw during shutdown");
            }
            vm.stopPrank();
        }
