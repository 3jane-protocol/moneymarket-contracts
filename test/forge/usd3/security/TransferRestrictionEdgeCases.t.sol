...
contract TransferRestrictionEdgeCases is Setup {
    ...
    function test_transferFrom_with_infinite_approval() public {
        ...
        vm.prank(bob);
        IERC20(address(usd3Strategy)).transferFrom(alice, charlie, aliceShares / 2);
        assertEq(IERC20(address(usd3Strategy)).balanceOf(charlie), aliceShares / 2);
    }
    ...
    function test_flash_loan_cannot_bypass_commitment() public {
        ...
        FlashLoanAttacker attacker = new FlashLoanAttacker(address(usd3Strategy), address(asset));
    ...
    function test_multiple_partial_transfers_after_commitment() public {
        ...
        assertEq(IERC20(address(usd3Strategy)).balanceOf(alice), initialShares / 4);
        assertEq(IERC20(address(usd3Strategy)).balanceOf(bob), initialShares / 2);
        assertEq(IERC20(address(usd3Strategy)).balanceOf(charlie), initialShares / 4);
    }
    ...
    function test_susd3_transfer_exactly_at_lock_expiry() public {
        ...
        assertEq(IERC20(address(susd3Strategy)).balanceOf(bob), susd3Shares / 2);
    }
    ...
    function test_susd3_dos_via_commitment_timestamp() public {
        ...
        uint256 susd3CommitmentTime = usd3Strategy.depositTimestamp(address(susd3Strategy));
        assertGt(susd3CommitmentTime, 0, "sUSD3 should have commitment timestamp");
        ...
        uint256 aliceUsd3Before = IERC20(address(usd3Strategy)).balanceOf(alice);

        // Approve sUSD3 to burn shares
        IERC20(address(susd3Strategy)).approve(address(susd3Strategy), aliceSusd3Shares);
        ...
        assertGt(aliceUsd3After, aliceUsd3Before, "Alice should have received USD3");
        assertEq(IERC20(address(susd3Strategy)).balanceOf(alice), 0, "Alice should have no sUSD3 left");
        vm.stopPrank();
        ...
        uint256 newCommitmentTime = usd3Strategy.depositTimestamp(address(susd3Strategy));
        assertGt(newCommitmentTime, susd3CommitmentTime, "Commitment should be extended");
        ...
        emit log_string("TEST RESULT: sUSD3 withdrawals work correctly despite commitment timestamp!");
        emit log_string("Fix successfully prevents DoS attack by exempting sUSD3 from transfer restrictions");
    }
}
...
contract FlashLoanAttacker {
    ...
    function attack() external {
        ...
        IERC20(address(usd3)).transfer(msg.sender, IERC20(address(usd3)).balanceOf(address(this)));
    }
}
