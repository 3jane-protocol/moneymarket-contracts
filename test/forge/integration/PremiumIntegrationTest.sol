...
contract PremiumIntegrationTest is BaseTest {
    ...
    function testBorrowAccruesPremiumBeforeIncrease() public {
        ...
        uint256 borrowAssetsAfter = uint256(positionAfter.borrowShares).toAssetsUp(
    ...
    function testFirstBorrowWithExistingPremiumRate() public {
        ...
        (uint128 lastAccrualTime,, uint256 snapshot) = MorphoCredit(address(morpho)).borrowerPremiumDetails(id, BORROWER);
    ...
    function testRepayFullAmountWithPremium() public {
        ...
        uint256 totalDebt = uint256(positionAfterAccrual.borrowShares).toAssetsUp(
            ...
            marketAfterAccrual.totalBorrowShares
        );
    ...
    function testRepayPartialWithPremium() public {
        ...
        Position memory positionAfter = morpho.position(id, BORROWER);
        Market memory marketAfter = morpho.market(id);
        uint256 debtAfter = uint256(positionAfter.borrowShares).toAssetsUp(
            marketAfter.totalBorrowAssets,
            marketAfter.totalBorrowShares
        );
    ...
    function testRepaySnapshotUpdateAfterPremium() public {
        ...
        (uint128 lastAccrualTime,, uint256 snapshotAfter) = MorphoCredit(address(morpho)).borrowerPremiumDetails(id, BORROWER);
        assertEq(lastAccrualTime, block.timestamp);
    ...
    function testRepayMoreThanDebtWithPremium() public {
        ...
        uint256 actualDebt = uint256(positionBeforeRepay.borrowShares).toAssetsUp(
            marketBeforeRepay.totalBorrowAssets,
            marketBeforeRepay.totalBorrowShares
        );
    ...
    function testWithdrawSupplyWithBorrowerPremium() public {
        ...
        uint256 remainingValue = uint256(supplierPosAfter.supplyShares).toAssetsDown(
    ...
    function testWithdrawHealthCheckWithPremium() public {
        ...
        // Try to withdraw collateral - should fail due to accumulated premium
        vm.prank(BORROWER);
        vm.expectRevert(bytes(ErrorsLib.INSUFFICIENT_COLLATERAL));
        morpho.withdrawCollateral(marketParams, 2_000e18, BORROWER, BORROWER);
    }
    ...
    function testWithdrawBySupplierWithActivePremiums() public {
        ...
        vm.prank(borrower2);
        morpho.supplyCollateral(marketParams, collateralAmount, borrower2, "");
        vm.prank(borrower2);
    ...
    function testAccrueInterestDoesNotAccruePremium() public {
        ...
        (uint128 timeBefore,, uint256 snapshotBefore) = MorphoCredit(address(morpho)).borrowerPremiumDetails(id, BORROWER);
    ...
    function testPremiumAccrualAfterMarketInterest() public {
        ...
        Market memory market = morpho.market(id);
        uint256 finalDebt = uint256(position.borrowShares).toAssetsUp(
    ...
    function testMultipleBorrowersIndependentPremiums() public {
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            morpho.supplyCollateral(marketParams, collateralAmount, borrowers[i], "");
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            debts[i] = uint256(pos.borrowShares).toAssetsUp(
                market.totalBorrowAssets,
                market.totalBorrowShares
            );
        }
    ...
    function testBatchPremiumAccrualIntegration() public {
        ...
        // All borrow and get premium rates
        for (uint256 i = 0; i < 5; i++) {
            vm.prank(borrowers[i]);
            morpho.supplyCollateral(marketParams, collateralAmount, borrowers[i], "");
    ...
    function testZeroPremiumRateBehavior() public {
        ...
        Market memory marketBefore = morpho.market(id);
        uint256 debtBefore = uint256(posBefore.borrowShares).toAssetsUp(
            marketBefore.totalBorrowAssets,
            marketBefore.totalBorrowShares
        );
        ...
        Market memory marketAfter = morpho.market(id);
        uint256 debtAfter = uint256(posAfter.borrowShares).toAssetsUp(
            marketAfter.totalBorrowAssets,
            marketAfter.totalBorrowShares
        );
    ...
    function testFeeRecipientSharesFromPremium() public {
        ...
        Market memory market = morpho.market(id);
        uint256 feeValue = uint256(feePos.supplyShares).toAssetsDown(
            market.totalSupplyAssets,
            market.totalSupplyShares
        );
    ...
    function testZeroFeeMarketWithPremium() public {
        ...
        uint256 supplierValue = uint256(supplierPos.supplyShares).toAssetsDown(
            marketAfter.totalSupplyAssets,
            marketAfter.totalSupplyShares
        );
