...
contract PhantomLiquidityFuzzTest is BaseTest {
    ...
    function testFuzz_MarkdownCannotCreatePhantomLiquidity(
        ...
        // Record state before markdown
        Market memory mBefore = morpho.market(id);
        uint256 supplyBeforeMarkdown = mBefore.totalSupplyAssets;
        ...
        // Verify markdown reduces supply appropriately
        // The reduction should be capped at the borrower's actual debt
        uint256 borrowShares = morpho.position(id, BORROWER).borrowShares;
        uint256 actualDebt = borrowShares.toAssetsUp(mBefore.totalBorrowAssets, mBefore.totalBorrowShares);
        uint256 expectedReduction = markdownAmount > actualDebt ? actualDebt : markdownAmount;

        assertApproxEqAbs(
            supplyBeforeMarkdown - mAfter.totalSupplyAssets,
            expectedReduction,
            100, // Allow small rounding difference
            "Supply should reduce by markdown amount (capped at debt)"
        );
    ...
    function testFuzz_MarkdownWithTimeDelays(uint256 timeDelay, uint256 markdownPercent) public {
        // Bound inputs
        timeDelay = bound(timeDelay, 1 hours, 365 days);
