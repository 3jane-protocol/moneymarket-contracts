...
contract MorphoCreditTest is Test {
    ...
    function testSetCreditLineWithExistingPosition() public {
        ...
        vm.prank(supplier);
        morpho.supply(marketParams, 5000e18, 0, supplier, "");
        ...
        assertEq(borrowAssetsAtLastAccrual, 1000e18); // Initial borrow amount
    }
    ...
    function testAccrueBorrowerPremium() public {
        ...
        uint256 creditLineAmount = 2500e18; // Account for 80% LLTV and min borrow
    ...
    function testAccruePremiumsForBorrowers() public {
        ...
        vm.prank(borrower2);
        morpho.borrow(marketParams, 1000e18, 0, borrower2, borrower2);
        ...
        assertGt(borrowAssets2, 1000e18);
    ...
    function testBorrowTriggersSnapshotUpdate() public {
        ...
        uint256 creditLineAmount = 5000e18; // Account for 80% LLTV and min borrow
        _setCreditLineWithPremium(borrower, creditLineAmount, uint128(uint256(0.1e18) / 365 days)); // 10% APR
        ...
        assertGt(snapshot2, 2000e18); // More than just 1000 + 1000 due to accrued interest/premium
    }
    ...
    function testRepayTriggersSnapshotUpdate() public {
        ...
        // Add enough tokens for full repayment including accrued interest
        loanToken.setBalance(borrower, 2000e18);
        vm.prank(borrower);
        loanToken.approve(address(morpho), type(uint256).max);

        // Repay all shares to trigger snapshot update and avoid minBorrow issues
        vm.prank(borrower);
        morpho.repay(marketParams, 0, pos.borrowShares, borrower, "");
        ...
        // After full repayment, snapshot should be 0
        assertEq(snapshot, 0);
        // Check that borrower has no remaining debt
        Position memory finalPos = morpho.position(marketId, borrower);
        assertEq(finalPos.borrowShares, 0);
    }
    ...
    function testVerySmallPremiumAmount() public {
        ...
        vm.prank(borrower);
        morpho.borrow(marketParams, 1000e18, 0, borrower, borrower); // Min borrow amount
    ...
    function testSetBorrowerPremiumRateAccruesBaseInterestFirst() public {
        ...
        vm.prank(supplier);
        morpho.supply(marketParams, 10000e18, 0, supplier, "");

        // Set initial credit line without premium
        uint256 creditLineAmount = 12500e18; // Account for 80% LLTV
        _setCreditLineWithPremium(borrower, creditLineAmount, 0);

        vm.prank(borrower);
        morpho.borrow(marketParams, 2500e18, 0, borrower, borrower);
    ...
    function testAccrueBorrowerPremiumBelowThreshold() public {
        // With minBorrow of 1000e18, we can't test truly small amounts
        // This test now verifies that very low premium rates still work correctly
        uint256 supplyAmount = 5000e18;
        uint256 borrowAmount = 1000e18; // Min borrow
        uint128 premiumRatePerSecond = uint128(uint256(0.00001e18) / 365 days); // 0.001% APR - extremely low
        ...
        (uint128 timestampBefore,,) = MorphoCredit(address(morpho)).borrowerPremium(marketId, borrower);

        // Advance very short time to get minimal premium
        ...
        // The position may or may not change depending on rounding
    ...
    function testPremiumCalculationPositionDecreased() public {
        ...
        // Just advance a bit more but stay under 60
