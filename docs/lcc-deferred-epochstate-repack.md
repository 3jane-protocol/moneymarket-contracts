...
The reserve arithmetic above is relative to the 24,208-byte artifact at `cf563d94`. Later size reductions change it: the dead-state removal landed in the same change series shrank the artifact to 23,955 bytes, against which the default-sequence RiskConfig 6→3 variant projects to roughly 24,311 bytes. That projection clears the ≥200-byte EIP-170 reserve (~265 bytes) but **exceeds the CI-enforced 24,276-byte release ceiling** (`MAX_RUNTIME_BYTES` in `scripts/check-lcc-release-artifact.js`) — re-opening any variant requires it to fit under the release ceiling, or an explicit owner decision to move the ceiling. **Re-measure the variants against the current artifact and the current ceiling before treating any per-variant verdict here as binding**; only the measured per-variant costs are frozen facts.
