...
optimizer runs, and no metadata bytecode hash. Its measured runtime is 24,126 bytes, 150 bytes below the internal ceiling and 450 bytes below EIP-170.
...
type, error, and event libraries are compiled internally and require no deployment or link addresses. The layout gate
