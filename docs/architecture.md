...
- Funding routes the funding asset through USD3 into the notification vault for the funder. Third parties may fund a user's obligation push-style via `fundCall(address)` (the caller pays; all proceeds accrue to the user). If USD3 or the notification vault cannot deliver, the funding transaction reverts and can be retried while the funding window remains open.
