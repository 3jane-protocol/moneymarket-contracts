...
- `src/lcc/LCCVaultFactory.sol`: owner-gated `LCCVault` deployment (`createVault` restricted to the immutable factory owner); registry membership records owner-vetted provenance. The `LCCVault` constructor is public, so the registry is a provenance signal rather than a deployment gate.
