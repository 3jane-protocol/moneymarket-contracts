...
- Funding routes the funding asset into USD3 for the funder, or into an internal escrow when USD3 lacks deposit capacity (`depositEscrowedFunding` pushes escrow into USD3 permissionlessly; escrow is refundable to the funder only under terminal shutdown). Third parties may fund a user's obligation push-style via `fundEpochCallFor` (the caller pays; all proceeds accrue to the user).
