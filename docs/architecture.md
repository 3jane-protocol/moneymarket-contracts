...
- Funding routes the funding asset through USD3 into the notification vault for the funder. Third parties may fund a user's obligation push-style via `fundEpochCall(uint256, address)` (the caller pays; all proceeds accrue to the user). If USD3 or the notification vault cannot deliver, the funding transaction reverts and can be retried while the funding window remains open.
- Exits are full-account, irrevocable, assigned first-fit to per-epoch maturity buckets capped at a percentage of the protocol commitment cap; exiting accounts remain liable until maturity.
