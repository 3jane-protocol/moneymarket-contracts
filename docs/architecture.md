...
- Build constraint: `LCCVault` is compiled for Shanghai with solc `0.8.35`, via IR, and a per-file optimizer setting of 500 runs in both Foundry and Hardhat. The measured runtime is 24,014 bytes, leaving 562 bytes below EIP-170; `yarn build:forge:size` is the release size gate.
