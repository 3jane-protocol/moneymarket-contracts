...
contract sUSD3Test is Setup {
    ...
    function setUp() public override {
        ...
        bytes memory susd3InitData = abi.encodeWithSelector(sUSD3.initialize.selector, susd3Asset, management, keeper);
    ...
    function test_initialization() public {
        ...
        assertEq(IUSD3(address(susd3Strategy)).performanceFeeRecipient(), management);
    ...
    function test_subordinationRatio_exactBoundary() public {
        ...
        assertApproxEqAbs(availableLimit, maxSusd3Allowed, 10e6, "Should allow up to exact ratio");

        // Bob deposits close to the limit (slightly less to avoid hitting exact limit)
        uint256 depositAmount = availableLimit > 10e6 ? availableLimit - 10e6 : availableLimit;
        vm.startPrank(bob);
        ERC20(address(usd3)).approve(address(susd3Strategy), depositAmount);
        ...
        uint256 susd3Usd3Holdings = ERC20(address(usd3)).balanceOf(address(susd3Strategy));
        ...
        assertLt(limitAfter, 100e6, "Should have very limited deposit capacity left");
    }
    ...
    function test_subordinationRatio_afterLoss() public {
        ...
        // Get initial amounts
        uint256 initialSusd3Balance = ERC20(address(susd3Strategy)).totalSupply();
        uint256 usd3Total = ERC20(address(usd3)).totalSupply();
        ...
        uint256 currentRatio = (initialSusd3Balance * 10000) / (usd3Total + initialSusd3Balance);
    ...
    function test_subordinationRatio_zeroSupply() public {
        ...
        assertLt(availableLimit, type(uint256).max, "Should have a limit based on USD3 supply");
        ...
        assertApproxEqAbs(limitAfterDeposit, maxSusd3, 10e6, "Should calculate correct limit based on ratio");
    }
