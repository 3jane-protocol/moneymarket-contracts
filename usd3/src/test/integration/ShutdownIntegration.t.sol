...
contract ShutdownIntegrationTest is Setup {
    ...
    function test_usd3_susd3_shutdown_interaction() public {
        ...
        vm.startPrank(alice);
        uint256 aliceUsd3Balance = IERC20(address(usd3Strategy)).balanceOf(alice);
        IERC20(address(usd3Strategy)).approve(address(susd3Strategy), aliceUsd3Balance);

        // Check subordination limit
        uint256 maxSusd3 = susd3Strategy.availableDepositLimit(alice);
        uint256 depositAmount = aliceUsd3Balance > maxSusd3 ? maxSusd3 : aliceUsd3Balance / 2;
        ...
        vm.startPrank(alice);
        IERC20(address(susd3Strategy)).approve(address(susd3Strategy), susd3Shares);
    ...
    function test_multi_user_shutdown_withdrawal() public {
        ...
        IERC20(address(usd3Strategy)).approve(address(susd3Strategy), aliceUsd3);
        uint256 maxDeposit = susd3Strategy.availableDepositLimit(alice);
        uint256 aliceDepositAmount = aliceUsd3 / 2 > maxDeposit ? maxDeposit : aliceUsd3 / 2;
        ...
        vm.startPrank(alice);
        uint256 aliceWithdrawnSusd3 = susd3Strategy.redeem(aliceSusd3, alice, alice);
        uint256 aliceRemainingUsd3 = IERC20(address(usd3Strategy)).balanceOf(alice);
        uint256 aliceWithdrawnUsd3 = usd3Strategy.redeem(aliceRemainingUsd3, alice, alice);
    ...
    function test_minimum_deposit_withdrawal_cycle() public {
        ...
        uint256 toRedeemNow = partialShares > maxRedeemableNow ? maxRedeemableNow : partialShares;

        if (toRedeemNow > 0) {
            IERC20(address(usd3Strategy)).approve(address(usd3Strategy), toRedeemNow);
        ...
        uint256 toRedeem = remainingShares > maxRedeemable ? maxRedeemable : remainingShares;
        ...
        if (finalShares == 0) {
            ...
            assertGt(shares4, 0, "Can deposit small amount with existing balance");
        }
        vm.stopPrank();
    }
    ...
    function test_complex_shutdown_scenario() public {
        ...
        uint256 bobSusd3Amount = bobUsd3 / 2 > maxSusd3 ? maxSusd3 : bobUsd3 / 2;
        ...
        IERC20(address(usd3Strategy)).approve(address(susd3Strategy), charlieUsd3);
        uint256 charlieSusd3Amount = susd3Strategy.availableDepositLimit(charlie);
        ...
        vm.startPrank(bob);
        // Approve for redeem
        IERC20(address(susd3Strategy)).approve(address(susd3Strategy), bobSusd3);
        ...
        // Approve for redeem
        IERC20(address(usd3Strategy)).approve(address(usd3Strategy), bobRemainingUsd3);
        uint256 bobWithdrawnUsd3 = usd3Strategy.redeem(bobRemainingUsd3, bob, bob);
        ...
        if (charlieSusd3 > 0) {
            // Approve for redeem
            IERC20(address(susd3Strategy)).approve(address(susd3Strategy), charlieSusd3);
            charlieWithdrawnSusd3 = susd3Strategy.redeem(charlieSusd3, charlie, charlie);
            assertGt(charlieWithdrawnSusd3, 0, "Charlie withdraws sUSD3 despite cooldown");
        }

        uint256 charlieRemainingUsd3 = IERC20(address(usd3Strategy)).balanceOf(charlie);
        // Approve for redeem
        IERC20(address(usd3Strategy)).approve(address(usd3Strategy), charlieRemainingUsd3);
        uint256 charlieWithdrawnUsd3 = usd3Strategy.redeem(charlieRemainingUsd3, charlie, charlie);
    ...
    function test_shutdown_with_protocol_config_changes() public {
        ...
        IERC20(address(usd3Strategy)).approve(address(susd3Strategy), aliceUsd3);
        uint256 maxDeposit = susd3Strategy.availableDepositLimit(alice);
        uint256 depositAmount = aliceUsd3 / 2 > maxDeposit ? maxDeposit : aliceUsd3 / 2;
        ...
        IERC20(address(susd3Strategy)).approve(address(susd3Strategy), aliceSusd3);
        ...
        IERC20(address(usd3Strategy)).approve(address(usd3Strategy), remainingUsd3);
        uint256 withdrawnUsd3 = usd3Strategy.redeem(remainingUsd3, alice, alice);
        assertGt(withdrawnUsd3, 0, "Should withdraw USD3 despite config changes");
        vm.stopPrank();
    }
    ...
    function test_shutdown_partial_withdrawal() public {
        ...
        assertEq(IERC20(address(usd3Strategy)).balanceOf(alice), 0, "All shares redeemed");
        assertApproxEqRel(
