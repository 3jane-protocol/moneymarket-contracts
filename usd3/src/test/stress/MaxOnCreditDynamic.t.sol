...
contract MaxOnCreditDynamicTest is Setup {
    ...
    function test_decreaseMaxOnCreditWithExistingDeployment() public {
        ...
        uint256 morphoPositionAssets = morpho.expectedSupplyAssets(marketParams, address(usd3Strategy));
        uint256 expectedDeployment = (totalAssets * MAX_ON_CREDIT_50) / 10_000;

        assertApproxEqAbs(morphoPositionAssets, expectedDeployment, 1000e6, "Should deploy ~50% to Morpho");
        ...
        uint256 morphoPositionAssetsAfterDecrease = morpho.expectedSupplyAssets(marketParams, address(usd3Strategy));
        assertEq(
            morphoPositionAssetsAfterDecrease, morphoPositionAssets, "Should not immediately rebalance on decrease"
        );
        ...
        uint256 newTotalAssets = ITokenizedStrategy(address(usd3Strategy)).totalAssets();
        uint256 newExpectedDeployment = (newTotalAssets * MAX_ON_CREDIT_25) / 10_000;

        // Should deploy only to reach 25% limit
        uint256 morphoPositionAssetsAfterNewDeposit = morpho.expectedSupplyAssets(marketParams, address(usd3Strategy));
        assertLe(morphoPositionAssetsAfterNewDeposit, newExpectedDeployment + 1000e6, "Should respect new lower limit");
    }
    ...
    function test_increaseMaxOnCreditTriggersDeployment() public {
        ...
        uint256 totalAssets = ITokenizedStrategy(address(usd3Strategy)).totalAssets();
        uint256 morphoPositionAssetsBefore = morpho.expectedSupplyAssets(marketParams, address(usd3Strategy));
        uint256 expectedAt25 = (totalAssets * MAX_ON_CREDIT_25) / 10_000;

        assertApproxEqAbs(morphoPositionAssetsBefore, expectedAt25, 1000e6, "Should deploy ~25% initially");
        ...
        uint256 newTotalAssets = ITokenizedStrategy(address(usd3Strategy)).totalAssets();
        uint256 morphoPositionAssetsAfter = morpho.expectedSupplyAssets(marketParams, address(usd3Strategy));
        uint256 expectedAt75 = (newTotalAssets * MAX_ON_CREDIT_75) / 10_000;

        // Allow wider tolerance since deployment happens incrementally
        assertApproxEqRel(morphoPositionAssetsAfter, expectedAt75, 0.1e18, "Should deploy more to reach ~75%");
        assertGt(morphoPositionAssetsAfter, morphoPositionAssetsBefore, "Should have deployed additional funds");
    }
    ...
    function test_maxOnCreditZeroToNonZero() public {
        ...
        uint256 morphoPositionAssetsAtZero = morpho.expectedSupplyAssets(marketParams, address(usd3Strategy));
        assertEq(morphoPositionAssetsAtZero, 0, "Should not deploy anything at 0% MaxOnCredit");
        ...
        assertApproxEqRel(morphoPositionAssetsAfter, expected, 0.1e18, "Should deploy to 50% after changing from 0%");
    ...
    function test_maxOnCreditToOneHundredPercent() public {
        ...
        assertApproxEqAbs(morphoPositionAssets, totalAssets, 1000e6, "Should deploy ~100% to Morpho");
    ...
    function test_precisionInDeploymentCalculations() public {
        ...
        assertApproxEqAbs(morphoPositionAssets, expectedDeployment, 10, "Should handle odd amounts precisely");
    }
    ...
    function test_deploymentCalculationsWithFees() public {
        ...
        assertApproxEqRel(
            morphoPositionAssets, expectedDeployment, 0.15e18, "Should maintain deployment ratio after fees"
        );
    }
    ...
    function test_deploymentWithMinimalAmounts() public {
        ...
        assertApproxEqAbs(morphoPositionAssets, expectedDeployment, minimalAmount, "Should handle minimal amounts");
    }
    ...
    function test_rebalancingOnWithdrawal() public {
        ...
        ITokenizedStrategy(address(usd3Strategy)).redeem(withdrawShares, alice, alice);
        vm.stopPrank();
        ...
        uint256 morphoPositionAssetsAfter = morpho.expectedSupplyAssets(marketParams, address(usd3Strategy));
        uint256 expectedDeploymentAfter = (totalAssetsAfter * MAX_ON_CREDIT_50) / 10_000;

        assertLt(morphoPositionAssetsAfter, morphoPositionAssetsBefore, "Should have withdrawn from Morpho");
        // With fixed _tend, rebalancing should be precise
        assertApproxEqRel(
            morphoPositionAssetsAfter, expectedDeploymentAfter, 0.05e18, "Should maintain deployment ratio"
        );
    }
    ...
    function test_rebalancingAfterLoss() public {
        ...
        assertLt(totalAssetsAfter, totalAssetsBefore, "Should have reduced total assets");
    ...
    function test_maxOnCreditBoundaryValidation() public {
        ...
        bytes32 MAX_ON_CREDIT = keccak256("MAX_ON_CREDIT");
        protocolConfig.setConfig(MAX_ON_CREDIT, MAX_ON_CREDIT_100);
        assertEq(usd3Strategy.maxOnCredit(), MAX_ON_CREDIT_100, "Should accept 100%");

        // Should not revert with 0%
        protocolConfig.setConfig(MAX_ON_CREDIT, MAX_ON_CREDIT_0);
        assertEq(usd3Strategy.maxOnCredit(), MAX_ON_CREDIT_0, "Should accept 0%");
    }
    ...
    function test_deploymentWithMorphoLiquidityConstraints() public {
        ...
        assertTrue(morphoPositionAssets >= 0, "Deployment should handle Morpho constraints");
    }
    ...
    function test_deploymentConsistencyAcrossOperations() public {
        ...
        assertApproxEqAbs(deploymentRatio, MAX_ON_CREDIT_50, 100, "Should maintain consistent deployment ratio");
    }
    ...
    function test_maxOnCreditFromProtocolConfig() public {
        ...
        assertEq(usd3Strategy.maxOnCredit(), MAX_ON_CREDIT_75, "Should read maxOnCredit from ProtocolConfig");
    }
    ...
    function test_maxOnCreditProtocolConfigIntegration() public {
        ...
        assertEq(usd3Strategy.maxOnCredit(), 0, "Should return 0 when set to 0");

        // When set to a value, should use the configured value
        protocolConfig.setConfig(MAX_ON_CREDIT, MAX_ON_CREDIT_75);
        assertEq(usd3Strategy.maxOnCredit(), MAX_ON_CREDIT_75, "Should use configured value");

        // When set to 100%, should allow full deployment
        protocolConfig.setConfig(MAX_ON_CREDIT, MAX_ON_CREDIT_100);
        assertEq(usd3Strategy.maxOnCredit(), MAX_ON_CREDIT_100, "Should return 100% when set to 100%");
    }
}
