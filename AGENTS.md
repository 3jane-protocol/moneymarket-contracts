...
**Round 3 — A/M-15: `setPerformanceFee` stays uncheckpointed on purpose.** `syncTrancheShare` now reports at the stored fee before writing new fee bits, so a reprice can no longer be applied retroactively to profit earned under the old rate. The inherited `setPerformanceFee` remains reachable through the TokenizedStrategy fallback and still makes the uncheckpointed change. That residual is **accepted deliberately**, on two grounds: wrapping it costs a measured **+471 bytes** of USD3, and the wiped-junior-tranche procedure in `docs/operations-runbook.md` uses it as its one intentional escape hatch — the only state where routing pending profit to seniors is the correct outcome. Wrapping it later is therefore not a 471-byte edit; it requires designing and testing a replacement recovery mechanism first.

**Operational coupling — the production keeper relayer auto-syncs the tranche fee.** `syncTrancheShare` now reports, so an automatic sync is now also an automatic report. The runbook's wiped-tranche procedure depends on *no* report landing at the old nonzero fee, which means suspending the relayer's tranche-fee sync is a prerequisite step, not a precaution. `onlyKeepers` does not protect this on its own — the relayer holds a keeper role. Confirm the relayer is halted before step 1 and keep it halted until the fee bits read zero.
