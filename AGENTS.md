...
- `maxEpochs` can schedule a vault sunset: `0` means perpetual, otherwise epochs `0..maxEpochs-1` are callable and epoch `maxEpochs` starts a terminal withdraw-only phase. Exit assignment uses the same phase-aware `_nextUncalledEpoch()` anchor as deposit activation, rejects a returned maturity at or after a bounded facility's `maxEpochs` before tracking it, and, only when all 128 maturity keys are occupied, admits on the full-list aggregate bound `S + A <= N * C` into the least-loaded eligible bucket with earliest maturity as the tie-break. `claimRemainingMargin` is the wind-down claim for shutdown or terminal sunset, while `claimExitedMargin` remains the normal maturity-gated exit claim. `minCommitmentEpochs` (immutable, up to 64, 0 disables) gates `requestExit` from the later of the latest deposit activation and one epoch after the call that produced the latest nonzero return credit; the wind-down claim bypasses it.
...
- `src/lcc/LCCVaultFactory.sol`: non-upgradeable family authority, admission registry, and `BeaconProxy` deployer. Its deliberately unheld `DEFAULT_ADMIN_ROLE` makes two-step `transferOwnership`/`acceptOwnership` the sole `OWNER_ROLE` mutation path, and owner-role renunciation is blocked. `OWNER_ROLE` administers `LISTER_ROLE`, `BOUNCER_ROLE`, `GUARDIAN_ROLE`, and `DEPOSIT_OPERATOR_ROLE`. Listers set or clear presence-separated per-beneficiary `uint128` commitment-cap overrides; unset users fall back to the owner-set `defaultDepositorCap`, explicit zero remains an absolute denial across later default changes, and `type(uint128).max` is unlimited. A payer distinct from the beneficiary must hold the deposit-operator role; the payer-role gate and beneficiary's post-deposit active-plus-pending cap check run on every deposit, before the same-vault top-up short-circuit, while the one-vault policy and admissions module gate only new opens and reopens. The payer is never cap-checked, and the role check must remain before the cap check. The cap is enforced per vault at admission; family-wide sizing is derived only while the prospective one-open-vault policy remains enabled with a clean history. Disabling that policy preserves warm `vaultOf` recording but makes neither exclusivity nor an aggregate family-cap claim, so operators must reconcile both open positions and aggregate commitment against each user's live cap before re-enabling. The optional admissions module is a narrow static decision hook; registry writes remain in the factory. The beacon is public, but non-factory proxies are unsupported and cannot pass the factory deposit gate.
...
- The LCC module is Cancun-only and pinned to solc `0.8.35`; `LCCVault.sol` uses scoped `optimizer_runs = 150` (24,268 runtime bytes after exit-accounting extraction; 108 bytes below the 24,376-byte release ceiling; 308 bytes of EIP-170 headroom) while the factory compiles at the repo default. The active-only monolith measured 24,703 bytes, 127 bytes over EIP-170, so `LCCExitLib` remains required. The vault uses `ReentrancyGuardTransient`, so every deployment chain must support EIP-1153. Forge produces the sole canonical deployment artifact; linked-library addresses and bytecode authenticity are deployment-procedure checks rather than constructor guards. Hardhat uses a pinned stable solc-js `0.8.35` for its compile/test-only Cancun output. USD3 remains Shanghai and compiles at the repo-default optimizer runs, while the frozen `USD3_old` upgrade-test artifact stays pinned at `optimizer_runs = 200`.
...
These include commitments made on the record in the Guardian Audits review and owner-approved residual-risk
preconditions from feature review. They are enforced by owner discipline, not by code. Breaking one makes the
corresponding code fix a prerequisite, not an option.

**LCC delegated-deposit beneficiary consent.** `DEPOSIT_OPERATOR_ROLE` authenticates the payer but does not prove
beneficiary consent. A role holder can make floor-sized deposits that refresh a beneficiary's
