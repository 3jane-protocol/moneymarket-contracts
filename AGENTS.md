...
- `src/lcc/LCCVaultFactory.sol`: owner-gated factory for `LCCVault` (`createVault` is restricted to the immutable factory owner); registry membership records owner-vetted provenance. The `LCCVault` constructor itself remains public, so the registry is a provenance signal, not a deployment gate.
