...
contract SimplifiedWAUSDC is ERC4626 {
    ...
    ) ERC4626(IERC20(_aToken)) ERC20(_name, _symbol) {
        aToken = _aToken;
...
contract SimplifiedUSD3 is ERC4626 {
    ...
    constructor(
        ...
        string memory _name,
        string memory _symbol
    ) ERC4626(IERC20(_waUSDC)) ERC20(_name, _symbol) {
    ...
    function _withdraw(
        ...
        require(
            block.timestamp >= depositTimestamp[owner] + minCommitmentTime,
...
contract SimplifiedSUSD3 is ERC4626 {
    ...
    constructor(
        ...
        string memory _name,
        string memory _symbol
    ) ERC4626(IERC20(_usd3)) ERC20(_name, _symbol) {}
    
    function startCooldown(uint256 shares) external {
        require(balanceOf(msg.sender) >= shares, "Insufficient balance");
        
        cooldowns[msg.sender] = UserCooldown({
            cooldownEnd: uint128(block.timestamp + cooldownDuration),
    ...
    function _withdraw(
        ...
        require(
            block.timestamp <= cooldown.cooldownEnd + withdrawalWindow,
