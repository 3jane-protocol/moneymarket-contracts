...
contract LossShareCalculationTest is Setup {
    ...
    function test_loss_calculation_bug() public {
        ...
        uint256 initialTotalAssets = ITokenizedStrategy(address(usd3Strategy))
            .totalAssets();
        uint256 initialTotalSupply = ITokenizedStrategy(address(usd3Strategy))
            .totalSupply();
        uint256 initialSusd3Balance = ITokenizedStrategy(address(usd3Strategy))
        ...
        console2.log(
            "  Initial PPS:",
            (initialTotalAssets * 1e18) / initialTotalSupply
        );
        ...
        uint256 assetsInMorpho = morpho.expectedSupplyAssets(
            params,
            address(usd3Strategy)
        );
        ...
        uint256 preReportTotalAssets = ITokenizedStrategy(address(usd3Strategy))
            .totalAssets();
        uint256 preReportTotalSupply = ITokenizedStrategy(address(usd3Strategy))
            .totalSupply();
        uint256 preReportSusd3Balance = ITokenizedStrategy(
            address(usd3Strategy)
        ).balanceOf(address(susd3Strategy));
        ...
        uint256 expectedSharesBurned = (lossAmount * preReportTotalSupply) /
            preReportTotalAssets;
        console2.log(
            "\nExpected shares to burn (using pre-report values):",
        ...
        uint256 postReportTotalAssets = ITokenizedStrategy(
            address(usd3Strategy)
        ).totalAssets();
        uint256 postReportTotalSupply = ITokenizedStrategy(
            address(usd3Strategy)
        ).totalSupply();
        uint256 postReportSusd3Balance = ITokenizedStrategy(
            address(usd3Strategy)
        ).balanceOf(address(susd3Strategy));
        ...
        console2.log(
            "  Post-report PPS:",
            (postReportTotalAssets * 1e18) / postReportTotalSupply
        );

        // Calculate actual shares burned
        uint256 actualSharesBurned = preReportSusd3Balance -
            postReportSusd3Balance;
        ...
        uint256 debugCalc = (loss * preReportTotalSupply) /
            postReportTotalAssets;
        console2.log(
            "  If we calculate: loss * preSupply / postAssets = ",
            debugCalc
        );
        console2.log(
            "  This matches the actual burned amount (111M vs 100M expected)"
        );
        ...
        if (actualSharesBurned < expectedSharesBurned) {
            ...
            console2.log(
                "Difference:",
                expectedSharesBurned - actualSharesBurned
            );
            ...
            console2.log(
                "Difference:",
                actualSharesBurned - expectedSharesBurned
            );
        } else {
        ...
        uint256 expectedPPSAfterBurn = ((postReportTotalAssets) * 1e18) /
            (postReportTotalSupply);
        ...
        assertEq(
            actualSharesBurned,
            expectedSharesBurned,
            "Incorrect number of shares burned from sUSD3"
        );
    }
    ...
    function test_loss_exceeds_susd3_balance() public {
        ...
        if (susd3BalanceAfter > 0) {
            console2.log(
                "BUG: sUSD3 still has balance when it should be wiped out!"
            );
        }
    }
}
