...
contract SubordinationLimitBypassTest is Setup {
    ...
    function setUp() public override {
        ...
        TransparentUpgradeableProxy susd3Proxy = new TransparentUpgradeableProxy(
                ...
                abi.encodeCall(
                    sUSD3.initialize,
                    (address(usd3Strategy), management, keeper)
                )
            );
        ...
        uint256 bobUsd3Balance = ITokenizedStrategy(address(usd3Strategy))
            .balanceOf(bob);
        ...
        console2.log(
            "USD3 total supply:",
            ITokenizedStrategy(address(usd3Strategy)).totalSupply()
        );
        console2.log(
            "sUSD3 holdings:",
            ITokenizedStrategy(address(usd3Strategy)).balanceOf(
                address(susd3Strategy)
            )
        );
    }
    ...
    function test_normal_subordination_ratio_enforcement() public {
        uint256 totalSupply = ITokenizedStrategy(address(usd3Strategy))
            .totalSupply();
        uint256 susd3Holdings = ITokenizedStrategy(address(usd3Strategy))
        ...
        console2.log(
            "Current subordination ratio:",
            (susd3Holdings * MAX_BPS) / totalSupply,
            "bps"
        );
        ...
        assertTrue(
            availableLimit < totalSupply,
            "Should not allow withdrawing everything"
        );
    ...
    function test_mathematical_convergence_limit() public {
        ...
        uint256 currentSupply = ITokenizedStrategy(address(usd3Strategy))
            .totalSupply();
        assertTrue(
            theoreticalLimit < currentSupply,
            "Theoretical limit should be below current supply"
        );
    ...
    function test_subordination_limit_bypass_via_multiple_withdrawals() public {
        ...
        while (withdrawalCount < maxWithdrawals) {
            ...
            if (availableLimit == 0) {
                console2.log(
                    "No more withdrawals available after",
                    withdrawalCount,
                    "withdrawals"
                );
                break;
            ...
            uint256 newTotalSupply = ITokenizedStrategy(address(usd3Strategy))
                .totalSupply();
            uint256 currentSubRatio = (susd3Holdings * MAX_BPS) /
                newTotalSupply;
            ...
            console2.log(
                "  Distance from limit:",
                newTotalSupply - theoreticalLimit
            );
        }
        ...
        assertGe(
            finalTotalSupply,
            theoreticalLimit,
            "Total supply went below theoretical limit!"
        );
        ...
        assertLe(
            finalSubRatio,
            maxSubRatio,
            "Subordination ratio exceeded maximum!"
        );
        ...
        if (finalTotalSupply == theoreticalLimit) {
            console2.log(
                "[PASS] Fix correctly prevents going below theoretical limit"
            );
            // Try one more withdrawal to confirm it's blocked
            uint256 additionalLimit = usd3Strategy.availableWithdrawLimit(
                alice
            );
            assertEq(
                additionalLimit,
                0,
                "Should not allow withdrawals below theoretical limit"
            );
            console2.log(
                "[PASS] Additional withdrawals correctly blocked at limit"
            );
        } else {
            console2.log(
                "[PASS] Subordination bypass correctly prevented before reaching limit"
            );
        }
    }
    ...
    function test_multiple_users_bypass_attempt() public {
        ...
        uint256 charlieWithdrawAmount = Math.min(
            charlieNewLimit,
            charlieBalance
        );
        ...
        uint256 finalSupply = ITokenizedStrategy(address(usd3Strategy))
            .totalSupply();
        ...
        console2.log(
            "[PASS] Multiple users correctly prevented from bypassing subordination limit"
        );
    }
    ...
    function test_boundary_conditions() public {
        ...
        assertTrue(
            currentRatio <= maxSubRatio,
            "Should not exceed subordination ratio"
        );
