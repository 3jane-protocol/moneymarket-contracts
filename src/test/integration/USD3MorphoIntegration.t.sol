...
contract USD3MorphoIntegrationTest is Setup {
    ...
    function test_supplyToMorphoCredit() public {
        ...
        // Deposit to strategy
        vm.prank(SUPPLIER);
        uint256 shares = ITokenizedStrategy(address(usd3Strategy)).deposit(amount, SUPPLIER);

        // Check strategy supplied to Morpho
        assertEq(ITokenizedStrategy(address(usd3Strategy)).totalAssets(), amount);
    ...
    function test_borrowAndAccrueInterest() public {
        ...
        assertGt(totalAssetsAfter, supplyAmount, "Total assets should increase after report");
    }
    ...
    function test_lowLiquidityWithdrawal() public {
        ...
        vm.prank(SUPPLIER);
        uint256 requiredShares = ITokenizedStrategy(address(usd3Strategy)).previewWithdraw(availableLimit);
        ITokenizedStrategy(address(usd3Strategy)).approve(address(usd3Strategy), requiredShares);
        vm.prank(SUPPLIER);
        uint256 withdrawn = ITokenizedStrategy(address(usd3Strategy)).withdraw(availableLimit, SUPPLIER, SUPPLIER);
