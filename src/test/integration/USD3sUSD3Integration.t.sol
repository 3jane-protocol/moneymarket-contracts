...
contract USD3sUSD3IntegrationTest is Setup {
    ...
    function setUp() public override {
        ...
        protocolConfig = MockProtocolConfig(
            MorphoCredit(morphoAddress).protocolConfig()
        );
    ...
    function test_fullLifecycle_deploymentToLoss() public {
        ...
        uint256 perfFee = ITokenizedStrategy(address(usd3Strategy))
            .performanceFee();
        uint256 expectedFeeShares = (profit * perfFee) / 10_000;

        uint256 susd3USD3Balance = ERC20(address(usd3Strategy)).balanceOf(
            address(susd3Strategy)
        );
        ...
        uint256 susd3BalanceAfterLoss = ERC20(address(usd3Strategy)).balanceOf(
            address(susd3Strategy)
        );
        assertLt(
            susd3BalanceAfterLoss,
            susd3USD3Balance,
            "sUSD3 should absorb loss"
        );

        // Phase 7: Withdrawals
        // Check available withdrawal limit first
        uint256 availableForAlice = usd3Strategy.availableWithdrawLimit(alice);
        uint256 aliceSharesValue = ITokenizedStrategy(address(usd3Strategy))
            .convertToAssets(aliceUSD3Shares / 2);
        uint256 toWithdraw = availableForAlice < aliceSharesValue
            ? ITokenizedStrategy(address(usd3Strategy)).convertToShares(
                availableForAlice
            )
            : aliceUSD3Shares / 2;
        ...
        if (toWithdraw > 0) {
            vm.prank(alice);
            uint256 aliceWithdrawn = usd3Strategy.redeem(
                toWithdraw,
                alice,
                alice
            );
    ...
    function test_yieldDistribution_flow() public {
        ...
        uint256 susd3Assets = ITokenizedStrategy(address(susd3Strategy))
            .totalAssets();
        assertEq(
            susd3Assets,
            bobDepositAmount,
            "sUSD3 assets should match Bob's deposit"
        );
    }
    ...
    function test_performanceFeeUpdate_viaSyncTrancheShare() public {
        ...
        uint256 susd3Balance1 = ERC20(address(usd3Strategy)).balanceOf(
            address(susd3Strategy)
        );
        ...
        uint256 susd3Balance2 = ERC20(address(usd3Strategy)).balanceOf(
            address(susd3Strategy)
        );
        ...
        assertGt(
            feeIncrease,
            0,
            "sUSD3 should receive more fees with higher tranche share"
        );
    }
        ...
        for (uint256 i = 0; i < 10; i++) {
            ...
            if (totalSupply > 0) {
                ...
                assertLe(
                    subRatio,
                    1500,
                    "Subordination ratio should be maintained"
                );
            }
    ...
    function test_cooldownPeriod_integration() public {
        ...
        uint256 aliceSusd3Shares = ERC20(address(susd3Strategy)).balanceOf(
            alice
        );
        ...
        vm.prank(alice);
        uint256 withdrawn = ITokenizedStrategy(address(susd3Strategy)).redeem(
            shares,
            alice,
            alice
        );
    ...
    function test_emergencyShutdown_integration() public {
        ...
        if (aliceShares > 0) {
            ...
            assertGt(
                aliceWithdrawn,
                0,
                "Alice should withdraw during shutdown"
            );
        }
        ...
        if (bobSusd3Shares > 0) {
            ...
            uint256 maxRedeem = ITokenizedStrategy(address(susd3Strategy))
                .maxRedeem(bob);
            if (maxRedeem > 0) {
                uint256 sharesToRedeem = maxRedeem < bobSusd3Shares
                    ? maxRedeem
                    : bobSusd3Shares;
                vm.prank(bob);
                uint256 bobWithdrawn = ITokenizedStrategy(
                    address(susd3Strategy)
                ).redeem(sharesToRedeem, bob, bob);
                assertGt(
                    bobWithdrawn,
                    0,
                    "Bob should withdraw sUSD3 during shutdown"
                );
            }
        }
    }
