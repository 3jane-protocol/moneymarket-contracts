...
contract InvariantsTest is Setup {
    ...
    function invariant_totalSupplyConsistency() public {
        ...
        for (uint256 i = 0; i < actors.length; i++) {
            usd3SumOfBalances += ERC20(address(usd3Strategy)).balanceOf(
                actors[i]
            );
        }
        ...
        assertApproxEqAbs(
            ...
            usd3SumOfBalances,
            100,
            "USD3: Total supply != sum of balances"
        );
        ...
        for (uint256 i = 0; i < actors.length; i++) {
            susd3SumOfBalances += ERC20(address(susd3Strategy)).balanceOf(
                actors[i]
            );
        }
        ...
        assertApproxEqAbs(
            ...
            susd3SumOfBalances,
            100,
            "sUSD3: Total supply != sum of balances"
        );
    }
    ...
    function invariant_subordinationWithdrawalEnforcement() public {
        ...
        if (usd3Circulating < minUSD3Required) {
            ...
            assertFalse(
                true,
                "Subordination ratio violated - circulating USD3 below minimum"
            );
        }

        // Also verify that sUSD3 holdings don't exceed the maximum subordination ratio
        uint256 actualSubRatioBps = susd3Holdings > 0
            ? (susd3Holdings * 10_000) / usd3TotalSupply
            : 0;

        assertLe(
            actualSubRatioBps,
            maxSubRatio,
            "sUSD3 holdings exceed maximum subordination ratio"
        );
    }
    ...
    function invariant_sharePriceMonotonicity() public {
        ...
        if (ERC20(address(usd3Strategy)).totalSupply() > 0) {
            uint256 sharePrice = (ITokenizedStrategy(address(usd3Strategy))
                .totalAssets() * 1e18) /
                ERC20(address(usd3Strategy)).totalSupply();
            assertGe(
                sharePrice,
                0.99e18,
                "USD3 share price dropped significantly"
            );
        }
        ...
        if (ERC20(address(susd3Strategy)).totalSupply() > 0) {
            uint256 susd3Assets = ITokenizedStrategy(address(susd3Strategy))
                .totalAssets();
            uint256 susd3SharePrice = (susd3Assets * 1e18) /
    ...
    function invariant_noNegativeBalances() public {
        for (uint256 i = 0; i < actors.length; i++) {
            ...
            assertLe(
                usd3Balance,
                type(uint256).max / 2,
                "USD3 balance overflow"
            );

            // Check sUSD3 balance
            uint256 susd3Balance = ERC20(address(susd3Strategy)).balanceOf(
                actor
            );
            assertLe(
                susd3Balance,
                type(uint256).max / 2,
                "sUSD3 balance overflow"
            );
        }
    }
    ...
    function invariant_depositWithdrawSymmetry() public {
        ...
        for (uint256 i = 0; i < actors.length; i++) {
            ...
            if (usd3Shares > 0) {
                uint256 withdrawable = ITokenizedStrategy(address(usd3Strategy))
                    .previewRedeem(usd3Shares);
                assertGt(
                    withdrawable,
                    0,
                    "Cannot withdraw despite having shares"
                );
    ...
    function invariant_maxDepositRespected() public {
        ...
        uint256 usd3MaxDeposit = ITokenizedStrategy(address(usd3Strategy))
            .maxDeposit(address(this));
        if (!ITokenizedStrategy(address(usd3Strategy)).isShutdown()) {
            assertGt(
                usd3MaxDeposit,
                0,
                "USD3 maxDeposit is 0 when not shutdown"
            );
        }

        // Check sUSD3 max deposit respects subordination
        uint256 susd3MaxDeposit = ITokenizedStrategy(address(susd3Strategy))
            .maxDeposit(address(this));
        ...
        if (susd3Supply > 0 && usd3Supply > 0) {
            uint256 maxAllowedSusd3 = (usd3Supply * 1500) / 8500; // 15% ratio
            uint256 availableRoom = maxAllowedSusd3 > susd3Supply
                ? maxAllowedSusd3 - susd3Supply
                : 0;
