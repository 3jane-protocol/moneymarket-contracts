...
contract SubordinationRatioStressTest is Setup {
    ...
    function test_subordinationRatio_exactBoundary() public {
        ...
        assertEq(
            aliceLimit,
            0,
            "At exact 85/15 boundary, no withdrawals allowed"
        );
    ...
    function test_subordinationRatio_multipleWithdrawals() public {
        ...
        uint256 aliceLimit = strategy.availableWithdrawLimit(alice);
        if (aliceLimit > 0) {
            uint256 aliceSharesValue = strategy.convertToAssets(
                aliceShares / 2
            );
            uint256 aliceToWithdraw = aliceLimit < aliceSharesValue
                ? strategy.convertToShares(aliceLimit)
                : aliceShares / 2;
        ...
        if (bobLimit > 0) {
            ...
            uint256 bobToWithdraw = bobLimit < bobSharesValue
                ? strategy.convertToShares(bobLimit)
                : bobShares / 2;
        ...
        if (newTotalSupply > 0) {
            ...
            assertLe(
                currentSubRatio,
                DEFAULT_SUB_RATIO,
                "Subordination ratio should not exceed limit"
            );
        }
    }
    ...
    function test_subordinationRatio_flashLoanResistance() public {
        ...
        deal(
            ...
            whale,
            underlyingAsset.balanceOf(whale) + flashLoanAmount
        );
        ...
        if (whaleLimit > 0 && whaleShares > 0) {
            ...
            uint256 whaleToWithdraw = whaleLimit < whaleSharesValue
                ? strategy.convertToShares(whaleLimit)
                : whaleShares;
        ...
        if (finalTotalSupply > 0) {
            uint256 finalSubRatio = (finalSusd3Holdings * MAX_BPS) /
                finalTotalSupply;
            assertLe(
                finalSubRatio,
                DEFAULT_SUB_RATIO,
                "Ratio should be maintained after attack"
            );
        }
    }
    ...
    function test_subordinationRatio_emergencyShutdownBypass() public {
        ...
        assertGt(
            shutdownLimit,
            0,
            "Should be able to withdraw during shutdown"
        );
    ...
    function test_subordinationRatio_roundingErrors() public {
        ...
        if (firstUserShares > 1) {
            ...
            if (newTotalSupply > 0) {
                uint256 currentRatio = (susd3Holdings * MAX_BPS) /
                    newTotalSupply;
                assertLe(
                    currentRatio,
                    DEFAULT_SUB_RATIO + 1,
                    "Ratio maintained with rounding tolerance"
                );
            }
        }
    }
    ...
    function test_subordinationRatio_dynamicConfigUpdate() public {
        ...
        assertEq(
            limitAfter,
            0,
            "Should restrict withdrawal after ratio tightening"
        );
