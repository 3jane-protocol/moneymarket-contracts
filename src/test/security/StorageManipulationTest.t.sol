...
contract StorageManipulationTest is Setup {
    ...
    function test_validateStorageSlotCalculation() public {
        ...
        bytes32 totalSupplySlot = bytes32(
            uint256(BASE_SLOT) + TOTAL_SUPPLY_SLOT_OFFSET
        );
        bytes32 balancesSlot = bytes32(
            uint256(BASE_SLOT) + BALANCES_SLOT_OFFSET
        );
        ...
        bytes32 susd3BalanceSlot = keccak256(
            abi.encode(address(susd3Strategy), balancesSlot)
        );

        // Read directly from storage
        uint256 directBalance = uint256(
            vm.load(address(usd3Strategy), susd3BalanceSlot)
        );
        uint256 directTotalSupply = uint256(
            vm.load(address(usd3Strategy), totalSupplySlot)
        );
        ...
        assertEq(
            directBalance,
            susd3Balance,
            "Direct storage read should match interface"
        );
        assertEq(
            directTotalSupply,
            totalSupply,
            "Direct total supply read should match interface"
        );
    ...
    function test_directStorageManipulation() public {
        ...
        uint256 susd3BalanceBefore = ITokenizedStrategy(address(usd3Strategy))
            .balanceOf(address(susd3Strategy));
        ...
        bytes32 totalSupplySlot = bytes32(
            uint256(baseSlot) + TOTAL_SUPPLY_SLOT_OFFSET
        );
        vm.store(
            address(usd3Strategy),
            totalSupplySlot,
            bytes32(totalSupplyBefore - burnAmount)
        );

        // Update sUSD3's balance (balances mapping at slot 4)
        bytes32 balancesSlot = bytes32(
            uint256(baseSlot) + BALANCES_SLOT_OFFSET
        );
        bytes32 susd3BalanceSlot = keccak256(
            abi.encode(address(susd3Strategy), balancesSlot)
        );
        vm.store(
            address(usd3Strategy),
            susd3BalanceSlot,
            bytes32(susd3BalanceBefore - burnAmount)
        );

        // Validate changes
        uint256 susd3BalanceAfter = ITokenizedStrategy(address(usd3Strategy))
            .balanceOf(address(susd3Strategy));
        ...
        assertEq(
            susd3BalanceAfter,
            susd3BalanceBefore - burnAmount,
            "sUSD3 balance should be reduced"
        );
        assertEq(
            totalSupplyAfter,
    ...
    function test_storageManipulationConsistency() public {
        ...
        uint256 aliceBalance = ITokenizedStrategy(address(usd3Strategy))
            .balanceOf(alice);
        uint256 charlieBalance = ITokenizedStrategy(address(usd3Strategy))
            .balanceOf(charlie);
        uint256 susd3Balance = ITokenizedStrategy(address(usd3Strategy))
            .balanceOf(address(susd3Strategy));
        uint256 totalSupplyBefore = ITokenizedStrategy(address(usd3Strategy))
        ...
        uint256 aliceBalanceAfter = ITokenizedStrategy(address(usd3Strategy))
            .balanceOf(alice);
        uint256 charlieBalanceAfter = ITokenizedStrategy(address(usd3Strategy))
            .balanceOf(charlie);
        uint256 susd3BalanceAfter = ITokenizedStrategy(address(usd3Strategy))
            .balanceOf(address(susd3Strategy));
        uint256 totalSupplyAfter = ITokenizedStrategy(address(usd3Strategy))
    ...
    function test_burnMinimalAmount() public {
        ...
        if (reportedLoss > 0) {
            uint256 expectedBurn = ITokenizedStrategy(address(usd3Strategy))
                .convertToShares(reportedLoss);
            assertApproxEqAbs(
                sharesBurned,
                expectedBurn,
                1,
                "Should burn minimal shares representing loss"
            );
        }
    }
    ...
    function test_precisionInBurning() public {
        ...
        if (reportedLoss > 0) {
            uint256 susd3BalanceAfter = ITokenizedStrategy(
                address(usd3Strategy)
            ).balanceOf(address(susd3Strategy));
            uint256 sharesBurned = susd3BalanceBefore - susd3BalanceAfter;

            uint256 expectedShares = ITokenizedStrategy(address(usd3Strategy))
                .convertToShares(reportedLoss);
            ...
            assertApproxEqAbs(
                ...
                expectedShares,
                200,
                "Precision should be maintained in burning"
            );
        }
    }
    ...
    function test_burnOperationAtomicity() public {
        ...
        assertEq(
            balanceReduction,
            supplyReduction,
            "Balance and supply reductions should be equal (atomic)"
        );
    }
    ...
    function test_storageCompatibilityAcrossVersions() public {
        ...
        // Read directly from storage
        uint256 directBalance = uint256(
            vm.load(address(usd3Strategy), aliceBalanceSlot)
        );

        assertEq(
            directBalance,
            aliceBalance,
            "Storage slot calculation should match actual TokenizedStrategy layout"
        );
    }
