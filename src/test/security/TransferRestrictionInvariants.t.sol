...
contract TransferRestrictionInvariants is Setup {
            ...
            if (balance > 0 && depositTime > 0) {
                ...
                if (block.timestamp < depositTime + 7 days) {
                    ...
                    try IERC20(address(usd3Strategy)).transfer(address(1), 1) {
                        ...
                        assertTrue(
                            false,
                            "Transfer succeeded during commitment"
                        );
                    } catch {
                        // Expected behavior
                    }
    ...
    function invariant_total_supply_conservation() public {
        ...
        sumBalances += IERC20(address(usd3Strategy)).balanceOf(
            address(susd3Strategy)
        );
        ...
        for (uint256 i = 0; i < users.length; i++) {
            ...
            if (usd3Balance > 0 && depositTime > 0) {
                ...
                if (inCommitment) {
                    ...
                    vm.prank(user);
                    try
                        IERC20(address(usd3Strategy)).transfer(
                            address(0x123),
                            1
                        )
                    {
                        assertTrue(
                            false,
                            "USD3 transfer during commitment should fail"
                        );
                    } catch {
                        // Expected
                    }
                        ...
                        )
                    {
                        ...
                        assertTrue(
                            false,
                            "USD3 transfer to sUSD3 should work during commitment"
                        );
                    }
                }
            }
            ...
            uint256 susd3Balance = IERC20(address(susd3Strategy)).balanceOf(
                user
            );
            uint256 lockedUntil = susd3Strategy.lockedUntil(user);
...
contract TransferRestrictionHandler is Test {
    ...
    modifier useRandomUser(uint256 seed) {
        if (users.length == 0) {
            ...
            address newUser = address(
                uint160(uint256(keccak256(abi.encode(seed, "user"))))
            );
            users.push(newUser);
            isUser[newUser] = true;
    ...
    function transfer(
        ...
        if (users.length < 2) {
            address newUser = address(
                uint160(
                    uint256(keccak256(abi.encode(recipientSeed, "recipient")))
                )
            );
            if (!isUser[newUser]) {
                users.push(newUser);
    ...
    function addUser(uint256 seed) public {
        address newUser = address(
            uint160(
                uint256(keccak256(abi.encode(seed, block.timestamp, "newUser")))
            )
        );
